...
class PointLK(torch.nn.Module):
    ...
    def do_forward(net, p0, p1, maxiter=10, xtol=1.0e-7, p0_zero_mean=True, p1_zero_mean=True):
        ...
        if p0_zero_mean:
            ...
            a0 = a0.clone()  # Clonar para evitar conflito de memória
            a0[:, 0:3, 3] = p0_m  # Atribuição sem conflito de memória
        ...
        if p1_zero_mean:
            ...
            a1 = a1.clone()  # Clonar para evitar conflito de memória
            a1[:, 0:3, 3] = -p1_m  # Atribuição sem conflito de memória
...
#EOF
# Alteração feita no novo projeto mudança de variaveis.
